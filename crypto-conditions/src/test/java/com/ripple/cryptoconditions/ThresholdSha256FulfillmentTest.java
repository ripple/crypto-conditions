...
public class ThresholdSha256FulfillmentTest extends AbstractFactoryTest {
  ...
  /**
   * Tests concurrently creating an instance of {@link ThresholdSha256Fulfillment}. This test validates the fix for
  ...
  /**
   * Tests weighting where at least 2 fulfillments are required, but 1 party has two votes. This is comparable to an
   * escrow transaction, where each party to the escrow has only a single vote, and both votes are required to fulfill a
   * transaction, but the escrow provider can supply two votes to force a decision.
   */
  @Test
